...
export const FLOORS = Array.from({ length: 17 }, (_, i) => String(i + 2)) // 2 ~ 18（C、D 棟）
export const FLOORS_HIGH = Array.from({ length: 20 }, (_, i) => String(i + 2)) // 2 ~ 21（A、B 棟樓層較高）
export const SUB_UNITS = Array.from({ length: 20 }, (_, i) => String(i + 1)) // 1 ~ 20

// A、B 棟樓層蓋得比 C、D 棟高，選這兩棟時樓層下拉選單要放寬到 21 樓
export function getFloors(building: string): string[] {
  return building === 'A棟' || building === 'B棟' ? FLOORS_HIGH : FLOORS
}
