...
export default function CoursePosterEditor({ course, initialImage, onClose }: Props) {
  ...
  const applyTheme = (bg: string, text: string) => {
    setBgColor(bg); setTextColor(text)
  }

  const applyCustomColor = (hex: string) => {
    setBgColor(hex); setTextColor(getTextColor(hex))
  }
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center bg-black/75 backdrop-blur-sm">
      {/* 手機：從底部滑出；桌機：置中 */}
      <div className="relative w-full md:max-w-4xl md:mx-4 flex flex-col md:flex-row bg-[#111] md:rounded-2xl rounded-t-2xl overflow-hidden shadow-2xl"
        ...
        <div ref={previewContainerRef}
          ...
          {!uploadedImage ? (
            ...
            <div ref={posterRef}
              ...
              {/* Image frame — drag inside here */}
              <div style={{ height: imageAreaH }}
                ...
                onWheel={onWheel}>
                <img src={uploadedImage} alt="" draggable={false}
                  style={{
                    position: 'absolute',
                    left: '50%', top: '50%',
              ...
              <div className="flex-1 flex flex-col justify-between px-5 py-4">
                <div>
                  <span className="inline-block text-xs tracking-[0.18em] px-2.5 py-0.5 rounded-full mb-2"
                  ...
                  {course.instructor && (
                    <p className="text-base opacity-60" style={{ color: textColor }}>{course.instructor}</p>
                  )}
                </div>
                ...
                <div className="space-y-2 mt-3">
                  {(course.date || course.timeStart) && (
                    <div className="flex items-center gap-2 text-sm" style={{ color: textColor }}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>
                      <span className="opacity-80">{course.date}{course.timeStart ? ` ${course.timeStart}` : ''}{course.timeEnd ? `–${course.timeEnd}` : ''}</span>
                    </div>
                  )}
                  {course.location && (
                    <div className="flex items-center gap-2 text-sm" style={{ color: textColor }}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/><circle cx="12" cy="9" r="2.5"/></svg>
                      <span className="opacity-80">{course.location}</span>
                    </div>
                  )}
                  {course.suitableAge && (
                    <div className="flex items-center gap-2 text-sm" style={{ color: textColor }}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2M12 11a4 4 0 100-8 4 4 0 000 8z"/></svg>
                      <span className="opacity-80">{course.suitableAge}</span>
                    </div>
                  )}
                  {course.notes && (
                    <p className="text-xs opacity-40 leading-relaxed pt-0.5" style={{ color: textColor }}>{course.notes}</p>
                  )}
                  <div className="text-[10px] tracking-widest opacity-25 pt-2 border-t"
                    style={{ borderColor: textColor === '#1a1a1a' ? 'rgba(0,0,0,0.1)' : 'rgba(255,255,255,0.1)', color: textColor }}>
          ...
          <div>
            ...
            <div className="grid grid-cols-6 md:grid-cols-3 gap-2 mb-3">
              {PRESET_THEMES.map((t, i) => (
                <button key={i} onClick={() => applyTheme(t.bg, t.text)} title={t.bg}
                  className={`h-9 rounded-lg border-2 transition-all duration-150 ${bgColor === t.bg ? 'border-white scale-105' : 'border-transparent opacity-70 hover:opacity-100'}`}
