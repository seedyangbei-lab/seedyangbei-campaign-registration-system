    ...
    <div className="sticky top-0 z-40 w-full h-[52px] md:h-14 px-4 md:px-6 flex items-center justify-between bg-white shadow-[0px_4px_2px_rgba(0,0,0,0.03)]">
      ...
      {!lineUser ? (
        ...
        >
          ...
          <span className="text-xs md:text-base font-medium text-stone-700">LINE 登入</span>
        </button>
      ) : (
        <div className="flex items-center gap-2">
          {/* 大頭貼＋名字：手機版一律顯示；電腦版只在首頁顯示，內頁改成單純文字登出（依 Figma 內頁導覽列規格） */}
          <Link
            href="/profile"
            className={`flex items-center gap-2 h-[34px] bg-white border border-white rounded-xl px-2.5 shadow-[0px_1px_1.5px_rgba(0,0,0,0.1),0px_1px_1px_rgba(0,0,0,0.1)] ${variant === 'inner' ? 'md:hidden' : ''}`}
          >
            {lineUser.pictureUrl ? (
          ...
          >
            <span className="text-xs md:text-base text-stone-500">登出</span>
          </button>
        </div>
      )}
    </div>
